...
*/
(function () {
  if (window.DS09) return;   // idempotent: re-running the loader must not redeclare
...
function initBST(suffix = '', lockedOp = null) {
  ...
  const codeTemplates = {
    put: {
      title: 'C++ — put <span class="ic-badge" style="background:var(--accent2)">CODE</span>',
      html: `
...
`
    ...
    get: {
      title: 'C++ — get <span class="ic-badge" style="background:var(--accent2)">CODE</span>',
      html: `
<span class="line"><span class="kw">#include</span> &lt;string&gt;</span>
<span class="line"><span class="kw">struct</span> TreeNode {</span>
<span class="line">    std::string key, value;</span>
<span class="line">    TreeNode *leftChild = <span class="kw">nullptr</span>, *rightChild = <span class="kw">nullptr</span>;</span>
<span class="line">};</span>
<span class="line" data-l="1">TreeNode* <span class="fn">get</span>(<span class="kw">const</span> std::string&amp; key, TreeNode* currentNode) {</span>
<span class="line" data-l="2">    <span class="kw">if</span> (currentNode == <span class="kw">nullptr</span>) <span class="kw">return</span> <span class="kw">nullptr</span>;</span>
...
<span class="line" data-l="5">    <span class="kw">if</span> (key &lt; currentNode->key)</span>
...
`
  ...
  const lineMap = {
    put: { visitLT: 3, visitGTE: 7, goLeft: 5, goRight: 9, insertLeft: 6, insertRight: 11, foundEq: 2, notFound: null },
  ...
  function genOpSteps(op, key) {
    ...
    while (cur) {
      ...
      if (key === cur.key) {
        const updating = op === 'put';
        steps.push({type:'found', node:cur, path:path.slice(), status:updating ? `key ${key} 已存在：更新 value，size 不變 ✓` : `找到 ${key} ✓`, line:L.foundEq, result:updating ? 'UPDATED' : 'FOUND', cmp});
        return steps;
      }
      if (key < cur.key) {
        if (cur.left) {
